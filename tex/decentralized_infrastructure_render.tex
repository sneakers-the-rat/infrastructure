...
make them easier to use. For researchers, their disconnection makes the
differences in their APIs a relatively sizable technical challenge to
integrate together, a problem compounded by the incentives for
...
It's unlikely that we will solve the problem by teaching every scientist
good programming practices, but we can build experimental frameworks
that make analysis tools easier to build and use. Specifically, a shared
...
analysis framework might work by sketching the separation between the
metadata and code modules. Since we're considering modular analysis
elements, each module would need some elemental properties like the
parameters that define it, its inputs, outputs, as well as some
...
(eg. cached intermediate data) and ``transitions'' (eg. an analysis
operation), and their related workflow markup languages (eg.
\href{https://openwdl.org/}{WDL} or \cite{vanderaalstYAWLAnotherWorkflow2005} ).
...
Where I could implement the code for one of them by, for example,
providing a set of methods to implement the different parts of the node
(a la \href{https://luigi.readthedocs.io/en/stable/tasks.html}{luigi}).
This lets us implement the logic of the node directly in the method, but
also provides a very thin wrapper that we can place around existing
tools. Here I'll show an example that sets some of the metadata in the
preceding spec in the code --- since we assume that
\texttt{Example\_Framework} is only one of many that implements the
workflow syntax, our framework is designed to let people write nodes
easily and then export their metadata as-needed.

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{from}\NormalTok{ Example\_Framework }\ImportTok{import}\NormalTok{ Node, Param, Types}

\KeywordTok{class}\NormalTok{ Bin(Node):}
\NormalTok{    bin\_width }\OperatorTok{=}\NormalTok{ Param(default}\OperatorTok{=}\DecValTok{10}\NormalTok{)}

    \KeywordTok{def} \BuiltInTok{input}\NormalTok{(}\VariableTok{self}\NormalTok{, input\_1: Types[some\_typinglike\_example]):}
        \CommentTok{\# validate}
        \VariableTok{self}\NormalTok{.}\BuiltInTok{input} \OperatorTok{=}\NormalTok{ input\_1}

    \KeywordTok{def}\NormalTok{ process() }\OperatorTok{{-}\textgreater{}}\NormalTok{ typing[output\_type]:}
        \CommentTok{\# some stuff!}
        \ControlFlowTok{return}\NormalTok{ [answer]}
\end{Highlighting}
\end{Shaded}

Then I could describe some workflow like this, using some .wdl-like
pseudocode:
...
!! explain markup, make sure the input/output/etc. param names are
recursively valid with nodes.

Having kept the description of our data in particular abstract from the
implementation of the code and the workflow specification, we now have a
reusable workflow we can apply to all of our datasets! Assuming
literally zero abstraction and using the tried-and-true ``hardcoded
dataset list'' pattern, something like:
...
expansion is not lost on me. The magic is in the way this hypothetical
analysis framework and markup interact with our data system and emerging
federated metadata system --- The layers of abstraction here are worth
unpacking.
